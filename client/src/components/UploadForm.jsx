...
export default function FileUpload({taskId}) {
  ...
  const handleUploadSubmit = async (e) => {
    ...
    const uploadResponse = await fetch('http://localhost:8080/api/upload/task', {
      method: 'POST',
      body: formData,
    });
    ...
    // Attach file to task
    const patchRes = await fetch(`http://localhost:8080/api/v1/tasks/${taskId}`, {
      method: 'PATCH',
  ...
  const handleDelete = async () => {
    ...
    const deleteRes = await fetch(`http://localhost:8080/api/upload?filepath=${task.attachment}`, {
      method: 'DELETE',
    });
    ...
    const patchRes = await fetch(`http://localhost:8080/api/v1/tasks/${tasks._id}`, {
      method: 'PATCH',
      headers: {'Content-Type': 'application/json'},
  ...
  return (
    ...
    <form onSubmit={handleUploadSubmit} className="p-4">
      ...
      {task?.attachment && (
        ...
        <button onClick={handleDelete} className="ml-4 text-red-500 underline">
          Delete File
        </button>
        </div>
      )}
